...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for x_words in self.domains[x]:
            ...
            for y_words in self.domains[y]:
                if x_words[index[0]]==y_words[index[1]]:
                    flag=True
                    break
            if flag==False:
                temp_words.append(x_words)
                revised=True
        ...
        for var in assignment:
            ...
            for v in self.crossword.neighbors(var):
                if v not in assignment:
                    continue
                index=self.crossword.overlaps[var,v]
        ...
        for word in self.domains[var]:
            for v in self.crossword.neighbors(var):
                ...
                for w in self.domains[v]:
                    if word[index[0]]!=w[index[1]]:
                        count+=1
                domain[word]=count
        domain_list=[]
       
        for word in sorted(domain.items(),key= lambda item: item[1]):
            domain_list.append(word[0])
        return domain_list
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var not in assignment:
                temp=len(self.domains[var])
                if min==None or temp<min:
                    min=temp
                    min_variables.append(var)
        max_var=None
        for var in min_variables:
            temp=len(self.crossword.neighbors(var))
            if highest_degree==None or temp>highest_degree:
                highest_degree=temp
                max_var=var
